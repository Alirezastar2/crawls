...
### فروش هر محصول در فاصلهٔ دو برداشت

جزئیات محصول در API سایت فیلد تجمعی `saleCount` دارد؛ این فیلد در CSV کاتالوگ
نیست. برای ثبت مبنا و سپس محاسبهٔ افزایش شمارندهٔ هر محصول:

```bash
python main.py sales-snapshot --city 1
# چند ساعت یا یک روز بعد:
python main.py sales-snapshot --city 1
python main.py sales-report --city 1 --csv data/sales-report.csv
```

`first_count` و `last_count` در گزارش دو مقدار شمارنده و `delta` تفاضل آنهاست.
اولین برداشت از زمان فعال‌کردن این قابلیت مبناست؛ فروش گذشته را نمی‌توان از
`products.csv` بازسازی کرد. `saleCount` تعداد موجودی انبار نیست و کاهش آن
به‌عنوان بازنشانی/اصلاح داده جداگانه گزارش می‌شود. برداشت کامل برای هر محصول
یک درخواست جزئیات می‌فرستد و با مکث پیش‌فرض ۰٫۵ ثانیه، زمان‌بر است.
برای آزمون کوچک می‌توان از `--limit 10` استفاده کرد.
...
اگر کرال ناقص یا ناموفق باشد، خروجی CSV/JSON آن دوره بازنویسی نمی‌شود؛
ممکن است دیتابیس شامل داده‌های ذخیره‌شده تا پیش از توقف باشد، پس برای خروجی
هماهنگ، ابتدا یک کرال کامل موفق اجرا کنید.
